---
title: HouseOfTheDragonAshenCrownRhaenyra
created: 2026-08-13
updated: 2026-08-13
type: query
tags: [wa-campaign, wa-tutorial, wa-character, hotd, hardcore, dance-of-the-dragons]
sources: [raw/hotd-ashen-crown-source-link.md]
---
...
**What you're playing** — Queen Rhaenyra Targaryen, the Black Queen, post-Tumbleton. Bard (College of Swords) Level 11 with the custom **Dragonrider subclass** Levels 1-20. Riding **Syrax** (your gold she-dragon, Huge Level 11, HP 295/295, AC 18, 16d6 fire breath). Quad-Pillar dashboard: **Crown Stability 22% (Critical)**, **Treasury Liquidity 350 gp (Near Bankruptcy)**, **Paranoia & Tyranny 65 (Severe — Maegor with Teats)**, **Dragonseed Loyalty −95 (Ulf) to +60 (Addam)**.
...
| **Mechanics (Jeff's Mechanical Precision)** | **CHECK THIS** — it is unchecked by default and must be checked for Quad-Pillar mechanics, the XP rule, and the Dragon class system to be honored by the AI DM (verified 2026-08-05) |

4. **Click Next → "Enter the World".** The AI DM will narrate the **Iron Throne scene** from Section 9 of the bible: Lord Corlys's ultimatum, the Shepherd's mob howling outside, the Captain bleeding from an eye socket, your son's death in the air between you. You'll then be offered a **3-option carousel + Freeform** decision. Pick and play.

---

## Copy-Paste Bible (≤16K-char slimmed version)
...
The default setup plays **Queen Rhaenyra canon at age 32 post-Tumbleton**. You can override anything in the **Character you want to play** field — e.g.:

- **Different era** — set the bible to play *before* Tumbleton (S2 finale or earlier) by editing the timeline in the bible's SECTION 1 anchor text
- **Different Quad-Pillar baseline** — adjust the four CS/TL/PTR/DL starting values in SECTION 7 if you want a less-crisis or more-crisis start
- **Different dragon** — swap Syrax for a different mount (caraxes, vermithor, seasmoke) by changing the L11 mount in SECTION 4
- **Different class** — the Bard chassis is fixed, but you can drop the Dragonrider subclass entirely and just play a Bard L11+ queen if you want a less-mechanical run
### Use a different HotD bible** — if you want to play the *Bastard's Claim* (Daemon's gender-ambiguous bastard on the eve of the Dance) instead, see [House of the Dragon — The Bastard's Claim](house-of-the-dragon-campaign.md)

### Editing the bible with an LLM

Paste the **LLM-wiki long-form source** (where this entire 9-section bible plus Appendices lives, with mechanical detail the slimmed wizard version drops) into any LLM chat and use:

> You are an expert narrative designer and Game Master. I have pasted the "House of the Dragon — The Ashen Crown (Rhaenyra)" custom-campaign template for WorldArchitect.AI. I want to edit as follows: **[ describe your change — e.g.g., "swap Syrax for Caraxes", "start at S2 finale instead of S3", "raise CS baseline to 50%", "make Daemon a full PC ally" ]**.
...
The bible is robust to most player-shaped edits. The wizard accepts the full ~16,000-character slimmed version above without truncation.

### L25 Divine Ascension — what to know

The Divine Ascension track at L25 is **optional and gated.** Most campaigns will not reach it. If you do reach it (campaign must run L11 → L25, roughly 14 Acts), pick a single Domain and the AI DM unlocks the right-of-petition mechanic, immunity to disease, and CHA cap removal. Syrax becomes your divine portfolio — if she dies, you take 1 level of permanent Exhaustion. This is the campaign's ultimate reward for sustained play.

### XP rule & god-mode boundary

The XP rule (`0.34 × Δthreshold` per die roll) is calibrated for sustained play. **God mode skips do not award XP** — narrator-driven cutscenes, time-skips, scripted sequences, and modal screens (level-up, character creation, downtime bookkeeping) all freeze XP. The DM must award XP per Section 8 of the bible, not as bonuses to other rolls. If the DM forgets, ask for the per-die-roll award.

### Troubleshooting

- **"Link doesn't exist" on the share URL** — the Gemini share may have been deleted or unpublished; the bible is built from the captured Gemini3.6 Flash campaign as of 2026-08-13
- **Setup walkthrough seems to contradict the field labels** — wizard uses generated IDs (`#wizard-campaign-title`); user-facing labels ("Campaign Title", "Campaign description prompt") are stable. Always use user-facing labels.
- **Use Default Fantasy World checkbox state** — leave UNCHECKED (default state; no action needed — verified 2026-08-05)
- **Mechanics (Jeff's Mechanical Precision) checkbox state** — CHECK (default unchecked — must check for Quad-Pillar / Dragon Class / XP rule to be honored — verified 2026-08-05)

---

## Sources

- [`raw/hotd-ashen-crown-source-link.md`](../raw/hotd-ashen-crown-source-link.md) — full 50,786-byte LLM-wiki source bible + Gemini3.6 Flash share-link capture
- [House of the Dragon — The Bastard's Claim](house-of-the-dragon-campaign.md) — companion bible (gender-ambiguous Daemon bastard, eve of Dance)
- [House of the Dragon (TV series, Wikipedia)](https://en.wikipedia.org/wiki/House_of_the_Dragon_(TV_series)) — show anchor (Season 3, 8 episodes, June 21–August 9, 2026)
- [Fire & Blood (Wikipedia)](https://en.wikipedia.org/wiki/Fire_%26_Blood) — book fallback (GRRM, 2018)
- [Dance of the Dragons (Wikipedia)](https://en.wikipedia.org/wiki/Dance_of_the_Dragons) — canonical conflict reference
- [`hermes-agent` worldarchitect-ai wiki source](https://github.com/jleechanorg/llm-wiki/blob/main/wiki/sources/house-of-the-dragon-ashen-crown.md) — full long-form source bible for LLM-assisted editing