...
## [2026-08-31] update | CampaignWizard + how-to-play — refresh to current 2-step Choose Type → Launch flow
- Rewrote `concepts/CampaignWizard.md` to describe the live 2-step flow (Choose Type, then Ready to Launch with editable Campaign Summary + Avatar + Enter the World). Removed the obsolete 3-step Setting → Character → Companion narrative.
- Mirrored the same 2-step rewrite into `entities/CampaignWizard.md` (was still describing the obsolete 3-step entity shape; leaving it stale would contradict the concept page and the `index.md` row).
- Rewrote `queries/how-to-play-worldai.md` so the wizard walkthrough (Steps 2-4) matches the live 2-step flow. Removed the now-fictional "three character-creation paths" Step 4 and the Ser Arion pre-filled character narrative (the wizard seeds a name only via the Dragon Knight preset card and otherwise leaves Character / Setting fields for the system to auto-generate after launch).
- Removed the stale 2026-06-20 wizard screenshots (`step2-wizard-setting-*`, `step3-wizard-launch-*`, `step4-character-creation-review-*`) from the page body and the `screenshots:` frontmatter. Added a "What the wizard looks like today" section that points to fresh captures at `~/evidence-wiki-wizard-refresh-2026-08-31/` (NOT committed to this public repo per the no-evidence convention). Kept the 2026-06-20 dashboard + opening scene + first action screenshots; those are still accurate.
- Updated `index.md` rows for the CampaignWizard entity and concept from "3-step" to "2-step".
- Note for the Phase-2 audit: the live wizard's Step 2 does NOT render AI Personalities / Options / Default-World checkboxes inside the Campaign Summary panel the way `docs/user-stories-ui/NEW-wizard-advanced-creation-options.md` claims — those checkboxes live on the legacy `#new-campaign-form` that the wizard hides. Either the UI stories are aspirational / outdated, or the work to wire them into the wizard was reverted. Flagged for the worldarchitect.ai doc-audit PR.
- Bumped `updated` in frontmatter for all three pages.
