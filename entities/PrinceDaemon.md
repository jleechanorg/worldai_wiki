---
title: PrinceDaemon
created: 2026-06-19
updated: 2026-06-19
type: entity
tags: [wa-character, wa-persona]
sources: []
---

# Prince Daemon

A specific NPC archetype from the House of the Dragon / Game of Thrones setting. A charming, dangerous prince with his own agenda.

See [[entities/Daemon]] for the personality archetype. Prince Daemon is a canonical example.

## In campaigns

Daemon appears in Game of Thrones-themed campaigns as:
- A potential ally (if your goals align).
- A dangerous rival (if your goals conflict).
- A wild card (if he's bored).

## Player tips

- **Engage carefully**: Daemon is fun to play with but dangerous.
- **Match his energy**: he respects boldness.
- **Don't trust fully**: he has his own goals.

See [[concepts/NPCRelationships]], [[concepts/CompanionPersonality]].
